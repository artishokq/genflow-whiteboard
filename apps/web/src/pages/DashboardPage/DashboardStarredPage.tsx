...
export function DashboardStarredPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const { boards: list } = await listBoardsRequest({ starred: true });
        if (!mounted) return;
        setBoards(list);
      } catch (e) {
        if (!mounted) return;
  ...
  if (filteredBoards.length === 0) {
    return (
      <p className={styles.emptyHint}>
        {normalizedSearch ? t("dashboard.emptySearch") : t("dashboard.emptyStarred")}
      </p>
    );
  }
          ...
          <div className={styles.userBoardMeta}>
            <div className={styles.userBoardTitleRow}>
              ...
              <span className={styles.boardRoleBadge}>
                {t(boardRoleLabelKey(board.myRole))}
              </span>
            </div>
          </div>
        </article>
